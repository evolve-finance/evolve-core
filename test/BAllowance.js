...
contract("BAllowance", function (accounts) {
  ...
  beforeEach(async () => {
    token = await BAllowance.new("Evo Token", "EVO");
  });

  it("borrowApprove", async () => {
    const receipt = await token.borrowApprove(other, TEST_AMOUNT, {
      from: user,
    });
  ...
  it("checkBorrowAllowance fail", async () => {
    ...
    await expectRevert(
      token.checkBorrowAllowance(user, other, TEST_AMOUNT),
      "Evo: BORROW_NOT_ALLOWED"
    );
    await token.borrowApprove(other, TEST_AMOUNT, { from: user });
    await expectRevert(
      token.checkBorrowAllowance(user, other, TEST_AMOUNT.add(new BN(1))),
      "Evo: BORROW_NOT_ALLOWED"
    );
  ...
  it("borrowPermit", async () => {
    console.log(userForEip712);
    console.log(otherForEip712);
    ...
    // });
    expectEqual(
      await token.borrowAllowance(userForEip712, otherForEip712),
      TEST_AMOUNT
    );
