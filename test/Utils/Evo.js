...
const EvoERC20 = artifacts.require('EvoERC20Harness');
...
const EvoCallee = artifacts.require('EvoCallee');
...
async function makeErc20Token(opts = {}) {
	...
	return await EvoERC20.new(name, symbol);
}
...
async function makeEvoPriceOracle(opts = {}) {
...
async function makeFactory(opts = {}) {
	...
	const uniswapV2Factory = opts.uniswapV2Factory || await makeUniswapV2Factory(opts);
	const evoPriceOracle = opts.evoPriceOracle || await makeEvoPriceOracle(opts);
	const factory = await Factory.new(admin, reservesAdmin, bDeployer.address, cDeployer.address, evoPriceOracle.address);
	return Object.assign(factory, {obj: {admin, reservesAdmin, bDeployer, cDeployer, uniswapV2Factory, evoPriceOracle,
