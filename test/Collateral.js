...
contract('Collateral', function (accounts) {
	...
	describe('getPrices', () => {
		...
		before(async () => {
			...
			await collateral.setPriceOracle(factory.obj.evoPriceOracle.address);
		...
		[
			...
			it(`getPrices for ${JSON.stringify(testCase)}`, async () => {
				...
				await factory.obj.evoPriceOracle.setPrice(underlying.address, uq112(priceOracle));
		...
		it(`fail if price(0|1) <= 100`, async () => {
			...
			await factory.obj.evoPriceOracle.setPrice(underlying.address, priceUQ112);
			await underlying.setTotalSupply(totalSupply);
			await underlying.setReserves(reserve0, reserve1);
			await expectRevert(collateral.getPrices(), 'Evo: PRICE_CALCULATION_ERROR');
	...
	[
		...
		describe(`Collateral tests for ${JSON.stringify(testCase)}`, () => {
			...
			it(`transfer:fail`, async () => {
				...
				await expectRevert(collateral.transfer(address(0), transferAmount, {from: user}), 'Evo: INSUFFICIENT_LIQUIDITY');
			});
	...
	describe('seize', () => {
		...
		it(`fail if msg.sender is not borrowable`, async () => {
			await expectRevert(collateral.seize(address(0), address(0), '0'), "Evo: UNAUTHORIZED");
		});
	...
	describe('flash redeem', () => {
		...
		before(async () => {
			...
			callee = (await EvoCallee.new(recipient.address, collateral.address)).address;
		});
		...
		it('redeem fails if redeemTokens is not enough', async () => {
			...
			await expectRevert(collateral.flashRedeem(user, redeemAmount, '0x'), 'Evo: INSUFFICIENT_REDEEM_TOKENS');
		});
	...
	describe('reentrancy', () => {
		...
		it(`borrow reentrancy`, async () => {
			await expectRevert(collateral.flashRedeem(receiver, '0', encode(['uint'], [1])), 'Evo: REENTERED');
			await expectRevert(collateral.flashRedeem(receiver, '0', encode(['uint'], [2])), 'Evo: REENTERED');
			await expectRevert(collateral.flashRedeem(receiver, '0', encode(['uint'], [3])), 'Evo: REENTERED');
			await expectRevert(collateral.flashRedeem(receiver, '0', encode(['uint'], [4])), 'Evo: REENTERED');
			await expectRevert(collateral.flashRedeem(receiver, '0', encode(['uint'], [5])), 'Evo: REENTERED');
