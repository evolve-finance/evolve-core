...
contract('Borrowable', function (accounts) {
	...
	describe('borrow and repay', () => {
		...
		async function makeBorrow(params) {
			...
			if (maxBorrowable.lt(expectedAccountBorrows)) {
				await expectRevert(borrowAction, 'Evo: INSUFFICIENT_LIQUIDITY');
				return false;
			}
		...
		before(async () => {
			...
			receiver = (await EvoCallee.new(recipient.address, underlying.address)).address;
		...
		it(`fail if cash is insufficient`, async () => {
			...
			await borrowable.sync();
			await expectRevert(borrowable.borrow(borrower, receiver, '1', '0x'), 'Evo: INSUFFICIENT_CASH');			
		});

		it(`fail if not allowed`, async () => {
			await underlying.setBalanceHarness(borrowable.address, '1');
			await borrowable.sync();
			await expectRevert(borrowable.borrow(borrower, receiver, '1', '0x'), 'Evo: BORROW_NOT_ALLOWED');			
		});
	...
	describe('liquidate', () => {
		...
		it(`fail if shortfall is insufficient`, async () => {
			...
			await expectRevert(borrowable.liquidate(borrower, liquidator), "Evo: INSUFFICIENT_SHORTFALL");		
		});
		...
		it(`fail if there is not enough collateral`, async () => {
			...
			await expectRevert(borrowable.liquidate(borrower, liquidator), "Evo: LIQUIDATING_TOO_MUCH");		
		});
	...
	describe('reentrancy', () => {
		...
		it(`borrow reentrancy`, async () => {
			await expectRevert(borrowable.borrow(address(0), receiver, '0', encode(['uint'], [1])), 'Evo: REENTERED');
			await expectRevert(borrowable.borrow(address(0), receiver, '0', encode(['uint'], [2])), 'Evo: REENTERED');
			await expectRevert(borrowable.borrow(address(0), receiver, '0', encode(['uint'], [3])), 'Evo: REENTERED');
			await expectRevert(borrowable.borrow(address(0), receiver, '0', encode(['uint'], [4])), 'Evo: REENTERED');
			await expectRevert(borrowable.borrow(address(0), receiver, '0', encode(['uint'], [5])), 'Evo: REENTERED');
			await expectRevert(borrowable.borrow(address(0), receiver, '0', encode(['uint'], [6])), 'Evo: REENTERED');
